...
import datetime
import pickle
import Predictor.read_data as read_data
...
def get_data(quote):
    ...
    try:
        ...
        df = pd.read_csv( f"./Predictor/SVM_Data/Data/{quote}_PRICE.csv")
        ...
        df = pd.DataFrame()
        for date in read_data.get_all_date():
            try:
                data = read_data.get_df(quote,date)
            except:
                continue
            
            print(date,end = '\r')
            #resample data to 5 minute
            df_date = data.copy().resample('5T').first()
            #create feature
        ...
        #save df
        df.to_csv(f"./Predictor/SVM_Data/Data/{quote}_PRICE.csv")
...
def get_VWAP(quote,df):
    ...
    for date in df.index.map(lambda t: t.date()).unique():
        try:
            ...
            OrderbookID = read_data.get_orderbook_id(quote,str(date).replace('-',''))
            #load current date's trade file
            trade_D = pd.read_csv(f"./Predictor/Data/{str(date).replace('-','')}/csv/{quote}_{OrderbookID}_trade.csv")
...
def get_SVM_df(quote):
    ...
    df.to_csv(f"./Predictor/SVM_Data/SVM_result/{quote}_SVM.csv")


def load_SVM_df(quote):
    df = pd.DataFrame()
    try:
        df = pd.read_csv(f"./Predictor/SVM_Data/SVM_result/{quote}_SVM.csv")
    except:
        get_SVM_df(quote)
        df = pd.read_csv(f"./Predictor/SVM_Data/SVM_result/{quote}_SVM.csv")
    return df
